...
public class FpcCompressor {
    final FcmPredictor predictor1;
    final DfcmPredictor predictor2;

    public FpcCompressor(int tableSize) {
	int logOfTableSize = LongMath.log2(tableSize, RoundingMode.UP);
	...
	int numZeroBytes1 = (header & 0x70) >> 4;
	if (numZeroBytes1 > 3) {
	    numZeroBytes1++;
	}
	int numZeroBytes2 = (header & 0x07);
	if (numZeroBytes2 > 3) {
	    numZeroBytes2++;
	}
	return 16 - numZeroBytes1 - numZeroBytes2;
    }
