...
/**
 * A fast int-long hash map.
 * 
 * Taken from
 * http://java-performance.info/implementing-world-fastest-java-int-to-int-hash-
 * map/
 */
public class IntLongMap {
    ...
    public IntLongMap(final int size, final float fillFactor) {
	...
	while (true) {
		...
		slot = (phiMix((int) k) & m_mask) << 1; // calculate the
							// starting slot for the
							// current key
	...
	for (int i = 0; i < oldCapacity; i += 2) {
	    final int oldKey = (int) oldData[i];
	    if (oldKey != FREE_KEY)
